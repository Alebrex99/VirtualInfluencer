...
"""_summary_: build a complete pipeline
    Pipeline Requirements
    The script must implement the following pipeline:
    Input directory scanning
    The script reads files from a directory named:

    Images
    The folder may contain multiple file types, therefore the script must filter only image files with the following extensions:
    .png
    .jpeg
    .jpg

    Any other file type must be ignored.
    
    Maximum number of images
    The script must process up to a predefined constant N.
    Example:
    MAX_IMAGES = N
    If the folder contains more than N valid images, only the first N images should be processed.

    Processing loop
    For each valid image:
    - Load the image
    - Send the image to the Nano Banana API
    - Generate three images sequentially

    The three outputs must correspond to:
    - High anthropomorphism
    - Medium anthropomorphism
    - Low anthropomorphism
    Each generation request should explicitly specify the intended anthropomorphism level.

    Prompt conditioning for Nano Banana
    When sending the request to Nano Banana, the prompt should describe the desired transformation.
    Guidelines:

    High anthropomorphism
    photorealistic
    extremely similar to the original human subject
    subtle identity variation
    preserve lighting, composition, and realism

    Medium anthropomorphism
    recognizable human face
    mild artificial features
    slight stylization or synthetic cues

    Low anthropomorphism
    cartoon-like
    stylized human avatar
    exaggerated or simplified features

    All generations must preserve the same resolution as the input image.

    Output storage
    After the three images are generated, they must be saved in the directory:
    Output_images
    The naming scheme must clearly associate the outputs with the original image.
    Example format:
    originalName_high.png
    originalName_medium.png
    originalName_low.png
    If the output directory does not exist, the script must create it automatically.

    Continue iteration
    After finishing the three generations for the current image, the pipeline must proceed to the next image.
    The process stops when either:

    N images have been processed
    or
    there are no more valid images in the folder.
    ...
    retry logic if the API request fails
    logging of processed images
    confirmation that output images are correctly written

    Expected Output from You

    Provide a complete implementation plan and code skeleton that includes:

    directory scanning
    image filtering
    iteration control
    Nano Banana API request structure
    prompt construction for the three anthropomorphism levels
    output file naming and saving
    error handling

    Use clear modular structure with functions.

    The code should be clean, well-commented, and ready to adapt to the Nano Banana API.

    Assume the API requires:

    an input image
    a prompt describing the transformation
...
def build_prompt(level: str) -> str:
    ...
    shared_prompt = (
        ...
        "lighting, and overall composition. Do not alter the underlying subject's gender or age, "
    ...
    if level == "high":
        return (
            ...
            + "Smooth the skin perfectly, remove blemishes, and enhance lighting for a flawless, "
    ...
    if level == "medium":
        return (
            shared_prompt
            + "Style: A 3D Video game digital illustration. "
            + "A 3D character model rendered in a modern game engine. "
            + "The subject has flawless digital skin and stylized CGI hair. "
            + "Maintain the exact pose, clothing, and framing of the source image. "
        )
    ...
    '''
        shared_prompt = (
            ...
            "Edit the input image while preserving the same person, pose, framing, "
            "lighting, composition, and original resolution. It's not changing the identity of the person" \
            "but rather applying a transformation to the image that results in a more or less anthropomorphized version of the same subject. "
        )
        if level == "high":
            return (
                shared_prompt
                + " Anthropomorphism level: HIGH. "
                + "The high anthropomorphism condition will aim for photorealistic output that closely approximates a real human photograph "
                + "Adjust the style to make the subject's skin look smoother"
            )
        if level == "medium":
            return (
                shared_prompt
                + " Anthropomorphism level: MEDIUM. "
                + "Keep a recognizable human face with mild artificial features and slight stylization."
            )
        if level == "low":
            return (
                shared_prompt
                + " Anthropomorphism level: LOW. "
                + "Create a cartoon-like stylized human avatar with simplified or exaggerated features."
            )
...
def generate_with_retry(client, model, image_path, prompt):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # DAI IL TEMPO DI BLOCCARE LA PIPELINE
            time.sleep(3)
...
def process_one_image(client, model, image_path):
    """
    Generate all three anthropomorphism variants for a single source image
