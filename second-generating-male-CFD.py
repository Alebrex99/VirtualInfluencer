...
ENHANCED_MEDIUM_PROMPT2 = (
    ...
    "ROLE: You are an expert image producer specializing in 3D character "
    "asset generation.\n"
    "TASK: Re-surface the attached image — an already-synthetic, beauty-enhanced "
    "character render of the subject — into a different target rendering style "
    "(Level MEDIUM anthropomorphism). The attached image is the canonical source "
    "of geometry, identity, photometry, and background; only the SHADER changes.\n\n"

    # ── SOURCE INPUT ────────────────────────────────────────────────────
    "SOURCE INPUT IMAGE: the attached image is the canonical source of TWO "
    "things and only the SHADER is allowed to change:\n"
    "  (a) GEOMETRIC + AESTHETIC IDENTITY — the enhanced facial structure, "
    "      proportions, features, hair, accessories, clothing, pose, and "
    "      framing exactly as they appear in the attached image.\n"
    "  (b) PHOTOMETRIC BLUEPRINT — light direction, shadow positions and "
    "      shadow softness, exposure value (EV), white balance, and the "
    "      pure white background of the attached image.\n"
    "The transformation re-renders the SURFACE SHADER on top of this "
    "blueprint. The shader is the only variable.\n\n"

    # ── IDENTITY LOCK ───────────────────────────────────────────────────
    "IDENTITY LOCK: the following attributes must be preserved exactly as "
    "they appear in the attached image, only re-rendered through the target shader:\n"
    "- Facial geometry: every feature's spatial coordinates, proportions, "
    "  asymmetries, and bone structure as currently rendered in the source.\n"
    "- Surface character: any marks, decals, or texture details present on "
    "  the source surface must be preserved as micro-features in the new "
    "  shader (do not invent new marks, do not erase existing ones).\n"
    "- Hair: exact hairline, parting, length, density, color, eyebrow shape "
    "  and density, eyelashes, beard/stubble pattern. The hair must be "
    "  re-rendered with the new shader.\n"
    "- Accessories: every earring, piercing, hair clip, glasses, necklace, "
    "  garment item — maintain the same geometry, placement, and color.\n"
    "- Identity attributes: apparent age, gender, ethnicity, and the "
    "  subject's current level of facial attractiveness — these must be "
    "  held constant; do not reduce, further enhance, or otherwise alter "
    "  the beauty optimization already present in the source.\n\n"
    ...
    # ── PHOTOMETRIC LOCK ────────────────────────────────────────────────
    "PHOTOMETRIC LOCK — preserve exactly (no shader instruction may override this):\n"
    "- Light direction and angle: identical to the attached image.\n"
    "- Shadow positions, shapes, edges, and softness: identical to the "
    "  attached image.\n"
    "- Exposure Value (EV) and overall brightness: identical to the "
    "  attached image — the output histogram should match the source "
    "  histogram in mid-tone placement and highlight roll-off.\n"
    ...
    # ── VARIABLE ────────────────────────────────────────────────────────
    "VARIABLE: only the SHADING MODEL and SURFACE RENDERING are allowed to "
    "change. Light direction is locked; how that light interacts with the "
    "skin is dictated by the target style.\n\n"

    # ── INSTRUCTION ─────────────────────────────────────────────────────
    "INSTRUCTION:\n"
    "1. Treat the subject in the attached image as a 3D digital mesh whose "
    "   geometry and appearance are already finalized.\n"
    "2. Re-surface this mesh using a synthetic-digital rendering engine in "
    "   the target style below.\n"
    "3. SURFACE PROTOCOL: re-render every surface (skin, hair, fabric) "
    "   using digital shaders consistent with the target style. The final "
    "   result must be a synthetic-digital reconstruction.\n\n"

    # ── TARGET RENDER STYLE — VERBATIM, UNCHANGED ──────────────────────
    "TARGET RENDER STYLE:\n"
    "RENDER STYLE — Level MEDIUM (Early-2000s Cinematic CGI / Uncanny Valley):\n"
    "Render in the aesthetic of The Polar Express (2004) or early "
    "performance-capture cinema, but replicate the exact Exposure Value (EV) of the base image. "
...
def main():
    ...
    # TESTING
    if isTest:
        test_image_paths = get_test_enhanced_images(INPUT_ENHANCED_MALE_DIR)
