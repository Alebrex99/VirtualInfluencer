...
def build_prompt(level: str) -> str:
    ...
    # ── Level 1 — Highly polished photorealism ──────────────────────────────
    if level == "high":
        return (
            shared_prompt
    ...
    # ── Level 2 — Enhanced realism with subtle CGI quality ──────────────────
    # Bridges between studio photography (high) and full game-engine render
    # (medium). Introduces a barely perceptible synthetic quality through skin
    # texture and lighting, while the image still reads as photographic.
    if level == "medium_high":
        return (
            shared_prompt
            + "Style: Enhanced digital realism with a subtle CGI quality. "
            + "Apply a high-end beauty retouch: poreless skin texture, soft subsurface "
            + "light scattering that gives the skin a slightly luminous, almost synthetic glow. "
            + "Refine the hair to look like a high-resolution CGI render. "
            + "The result should still appear photographic but with a noticeable digital "
            + "perfection that hints at computer generation rather than a real photograph."
        )

    # ── NEW Level 3 — 3D video game character, game-engine render ───────────────
    if level == "medium":
        return (
            shared_prompt
            + "Style: Enhanced digital realism with an evident-high CGI quality. "
            + "Apply a high-end beauty retouch: poreless skin texture, soft subsurface "
            + "light scattering that gives the skin a slightly luminous, almost synthetic glow. "
            + "Refine the hair to look like a high-resolution CGI render. "
            + "The result should still appear photographic but with a noticeable digital "
    ...
    # ── NEW Level 4 — Stylized 3D animation / semi-cartoon ──────────────────────
    # Bridges between the clean game-engine render (medium) and the fully flat
    # 2D cartoon (low). Pushes stylization further with exaggerated proportions
    # and a non-photorealistic shading model, while still using 3D volume.
    if level == "medium_low":
        return (
            shared_prompt
            + "Style: Stylized 3D animation in the tradition of modern animated feature films. "
            + "Apply a Pixar or DreamWorks-inspired rendering style: smooth, slightly exaggerated "
            + "facial proportions, large expressive eyes, soft cel-shaded lighting with strong "
            + "ambient occlusion, and a painterly color palette. "
            + "The result should look like a frame from a high-quality 3D animated movie, "
            + "clearly non-photorealistic but still three-dimensional and volumetric."
        )

    # ── Level 5 — 2D cartoon illustration, fully illustrated ────────────────
    if level == "low":
        return (
            shared_prompt
